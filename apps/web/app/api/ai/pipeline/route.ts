  ...
  try {
    ...
    try {
      ...
      try {
        ...
        reviewId = review?.id;
        aiCategorizationSaved = Boolean(reviewId);
        console.warn('[AI] AICategorization created with minimal Prisma fields due to error:', err?.message ?? err);
      } catch (err2: any) {
        try {
          const normalized = content.trim().replace(/\s+/g, ' ').slice(0, 500);
          const id = randomUUID();
          const cols: Array<{ column_name: string; is_nullable: string; data_type: string }> = await (prisma as any).$queryRawUnsafe(
          ...
          const present = (name: string) => cols.some((c) => c.column_name === name);
          const get = (name: string) => cols.find((c) => c.column_name === name);
          ...
          if (present('contentPreview')) addParamCol('contentPreview', normalized);
          if (present('suggestedFolder')) addParamCol('suggestedFolder', aiResponse.suggestedFolder);
          if (present('acceptedFolder')) addParamCol('acceptedFolder', aiResponse.suggestedFolder);
          if (present('confidence')) addParamCol('confidence', aiResponse.confidence ?? 0);
          ...
          const result = await (prisma as any).$executeRawUnsafe(sql, ...values);
          aiCategorizationSaved = result > 0;
          console.warn('[AI] AICategorization inserted via dynamic raw SQL; columns=', colNames.join(','), 'affected rows=', result);
        } catch (err3: any) {
          console.warn('[AI] Failed to persist AICategorization (all attempts):', err3?.message ?? err3);
        }
      }
    }
