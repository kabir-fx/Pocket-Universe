...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (galaxyName) {
      ...
      if (!ensured)
        return NextResponse.json(
          { error: "Failed to create/get new folder for img" },
          { status: 403 },
        );
    ...
    if (!ALLOWED_MIME.has(normalizedCT))
      return NextResponse.json({ error: "invalid img type" }, { status: 415 });
    if (buffer.length > MAX_IMG_SIZE)
      return NextResponse.json(
        { error: "img size too large" },
        { status: 413 },
      );
    ...
    if (uploadRes.error) {
      return NextResponse.json(
        { error: "Failed to upload image", message: uploadRes.error.message },
        { status: 500 },
      );
    }
    ...
    const availableModels = Object.keys(anyPrisma).filter(
      (k) => typeof anyPrisma[k] === "object" && anyPrisma[k]?.findMany,
    );
      ...
      return NextResponse.json(
        {
          ...
          message: `Prisma model 'Image' is not generated. Available models: ${availableModels.join(", ")}. Try: rm -rf apps/web/.next && restart dev server`,
        },
        { status: 500 },
      );
    }
    ...
    try {
      ...
      await anyPrisma2?.aICategorization?.create?.({
        data: {
          ...
          suggestedFolder:
            aiResult?.suggestedFolder ?? (galaxyRes?.name || "orphaned"),
